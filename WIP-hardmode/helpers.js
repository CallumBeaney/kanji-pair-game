...
function identifyRepeatUsableTiles(currentWord)
{
  ...
  for (i = 0; i < successfulKanjiButtonIDs.length; i++) {
    ...
    for (j = 0; j <= unPassedKanjis.length; j++)
    {
      ...
      if (checkerWord != currentWord && (checkerWord in wordList || invertedCheckerWord in wordList)){
  ...
  if (unPassedKanjiButtonIDs.length <= 2) 
  {
    for (a = 0; a < unPassedKanjis.length; a++) 
    {
      ...
      for (b = 0; b < allSuccessfulKanji.length; b++){
        ...
        if (checkword in wordList) {
          ...
          document.getElementById(successfulKanjiButtonIDs[b]).className = "kanjiButton finalState"; // make it blue
        }
      }  
    }
  }
...
function tryKanji(newKanji, id) {
  ...
  if (state.tries == 0){ 
    ...
    else if(document.getElementById(id).className == "kanjiButton finalState"){
      document.getElementById(id).className = "kanjiButton reSelected"; 
    } 
    else if(document.getElementById(id).className != "kanjiButton finalState"){  
